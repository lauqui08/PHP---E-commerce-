...
<?php
 ...
 if (isset($_GET['Filter'])) {
    ...
    if ($_GET['brand']=="all" AND $_GET['color']=="all" AND $_GET['type']=="all" AND $_GET['thickness']=="all") {
        # code...
        header("Location:http://localhost/ootb/member/yarns/");
    }
 }
  ?>
<head>
...
<div class="row">
            ...
            <div class="panel panel-primary">
              ...
              <div class="panel-heading">
                <div class="row">
                      ...
                      <div class="col-sm-4"><a href="http://localhost/ootb/member/yarns/" class="btn btn-xs btn-success pull-right"><span class="glyphicon glyphicon-refresh"></span></a></div>
  ...
  <?php
                       if (isset($_GET['Filter'])) {
                        ...
                        $prodlist = mysqli_query($con,"SELECT * FROM `ootb_products` WHERE product_brand LIKE'%$pbrand%' AND product_color LIKE'%$pcolor%' AND product_style LIKE'%$ptype%' AND product_thickness LIKE'%$pthickness%'");

                    }else{
                            $prodlist = mysqli_query($con,"SELECT * FROM `ootb_products`");
                    }
          ...
          <div class="row">
            ...
            <div class="col-sm-4">
                <a href="" class="btn btn-info viewprod" data-toggle="modal" data-target="#myModaledit" data-backdrop="static"
                             data-id="<?php echo $row['product_id']; ?>" data-sku="<?php echo $row['product_sku']; ?>" data-name="<?php echo $row['product_name']; ?>" data-color="<?php echo $row['product_color']; ?>" data-style="<?php echo $row['product_style']; ?>" data-thickness="<?php echo $row['product_thickness']; ?>" data-brand="<?php echo $row['product_brand']; ?>" data-price="<?php echo $row['product_price']; ?>" data-quantity="<?php echo $row['product_quantity']; ?>" data-details="<?php echo $row['product_details']; ?>" data-image="<?php echo $row['product_image']; ?>">Info...</a>
            </div>
        ...
        <div class="modal-body table-responsive">
              ...
              <form class="form-horizontal">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8">
                    <p class="form-control-static" ><span id="xpcolor"></span></p>
                  </div>
                </div>
                ...
                <div class="form-group">
                  <label class="control-label col-sm-4" for="thickness">Thickness:</label>
                  <div class="col-sm-8">
                    <p class="form-control-static" id="xpthickness"></p>
                  </div>
                </div>
            ...
            <div class="col-sm-5">
              <form action="<?php echo htmlspecialchars("transaction_exe.php");?>" method="GET">
                <div class="input-group">
                    ...
                    <input type="hidden" name="url_yarn" value='<?php $url="http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']; echo $url; ?>' />
...
<script type="text/javascript">
   $('.viewprod').click(function(){
            ...
            $('#xsku').text(sku);
              $('#disxsku').val(sku);
            $('#xpname').text(name);
            $('#xpcolor').text(color).css('color',color);;
            $('#xpstyle').text(style);
            $('#xpthickness').text(thickness);
            $('#xpbrand').text(brand);
            $('#xpprice').text(price);
              $('#xpprice2').val(price);
   ...
   $(".filteryarn").change(function(){
        ...
        $(location).attr('href','http://localhost/ootb/member/yarns/?Filter&brand='+brand+'&type='+type+'&thickness='+thickness+'&color='+color);
